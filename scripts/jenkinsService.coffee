...
#   HUBOT_JENKINS_URL
#   HUBOT_JENKINS_AUTH
process.env.HUBOT_JENKINS_URL = 'https://slack-it-jenkins.herokuapp.com/'
process.env.HUBOT_JENKINS_AUTH = 'admin:CodeGamesAdmin'
#
...
jenkinsLast = (msg) ->
    ...
    req.get() (err, res, body) ->
        if err
          ...
          try
            content = JSON.parse(body)
            d = new Date(content.timestamp).toLocaleString()
            response += "NAME: #{content.fullDisplayName}\n"
            response += "URL: #{content.url}\n"
            response += "DATE: #{d}\n"
            response += "DURATION: #{content.duration} ms\n"
